...
const productImgResize = async (req, res, next) => {
    ...
    await Promise.all(req.files.map(async (file) => {
        ...
        file.path = `public/images/products/${file.filename}`; // Update the file.path directly without using await
    }));
    next();
}
...
const blogImgResize = async (req, res, next) => {
    ...
    await Promise.all(req.files.map(async (file) => {
        ...
        fs.unlinkSync(file.path);
        file.path = `public/images/blogs/${file.filename}`; // Update the file.path directly without using await
    }));
    next();
}
